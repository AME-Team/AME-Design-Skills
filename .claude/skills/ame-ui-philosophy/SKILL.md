...
- 判断基準: Flexbox/Grid の相対配置だけで組めるか（→ HTML）、絶対座標やパスの計算が要るか（→ SVG）で選択する。
- HTML 実装はレスポンシブ・ダークモード対応が容易なため、座標計算が不要な限り優先する。
- SVG 実装時も色は `--color-primary` または `currentColor` 経由にし、`dark:` でのコントラスト維持を確保する（§4 準拠）。

### 3.3 図表の可変性（フォント・多言語・カラー対応）

図表（HTML/SVG いずれの実装でも）は、通常の UI コンポーネントと同じ可変性要件に従うこと。決め打ちの見た目で固定しない。

- **フォント**: 図表内のラベル・注釈テキストにも `font-family` を決め打ちせず、[[ame-ui-typography]] Skill の
  `--font-ui-ja-sans` / `--font-ui-en-sans` 等の CSS 変数を継承させる。SVG 内 `<text>` も
  `font-family: var(--font-ui-...)` を指定し、`locale`
  切替でフォントが自動的に追従するようにする。
...
- **カラー**: §3.2 の通り `--color-primary` / `currentColor` を使い、ライト/ダーク切替と 1
  ポイントカラー変更（§4.2）の両方に追従させる。
